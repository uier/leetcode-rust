fn main() {
    let tests = [
        (vec![1, 2, 4], 5),
        (vec![1, 4, 8, 13], 5),
        (vec![3, 9, 6], 2),
        (
            vec![
                9930, 9923, 9983, 9997, 9934, 9952, 9945, 9914, 9985, 9982, 9970, 9932, 9985, 9902,
                9975, 9990, 9922, 9990, 9994, 9937, 9996, 9964, 9943, 9963, 9911, 9925, 9935, 9945,
                9933, 9916, 9930, 9938, 10000, 9916, 9911, 9959, 9957, 9907, 9913, 9916, 9993,
                9930, 9975, 9924, 9988, 9923, 9910, 9925, 9977, 9981, 9927, 9930, 9927, 9925, 9923,
                9904, 9928, 9928, 9986, 9903, 9985, 9954, 9938, 9911, 9952, 9974, 9926, 9920, 9972,
                9983, 9973, 9917, 9995, 9973, 9977, 9947, 9936, 9975, 9954, 9932, 9964, 9972, 9935,
                9946, 9966,
            ],
            3056,
        ),
    ];
    let answers = [3, 2, 1, 73];
    for (test, expected_answer) in tests.into_iter().zip(answers) {
        let answer = Solution::max_frequency(test.0, test.1);
        assert_eq!(answer, expected_answer);
    }
}
struct Solution;
impl Solution {
    pub fn max_frequency(nums: Vec<i32>, k: i32) -> i32 {
        let mut nums = nums;
        nums.sort();
        let n = nums.len();
        let (mut r, mut cost, mut ans) = (0usize, 0, 1i32);
        for l in 0..n - 1 {
            while cost <= k {
                ans = ans.max((r - l + 1) as i32);
                if r + 1 >= n {
                    break;
                }
                cost += (r - l + 1) as i32 * (nums[r + 1] - nums[r]);
                r += 1;
            }
            cost -= nums[r] - nums[l];
        }
        ans
    }
}
